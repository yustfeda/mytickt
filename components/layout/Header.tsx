...
const NavIcon: React.FC<{
  ...
  href?: string;
}> = ({ label, icon, onClick, isActive = false, badgeCount = 0, isExternalLink=false, href="#" }) => {
    const commonProps = {
...
const Header: React.FC<HeaderProps> = ({ toggleSidebar, isSidebarOpen, navigate, user, logout, onLoginClick, onRegisterClick, onAdminClick, isAdminAuthenticated, adminLogout, activeAdminPage, setActiveAdminPage, unreadCount, customButtons }) => {
        ...
        return (
            <div className="flex items-center gap-2">
                ...
                {customButtons.filter(b => b.isActive).map(button => (
                  <NavIcon key={button.id} isExternalLink href={button.url} onClick={() => {}} icon={button.icon} label={button.name} />
                ))}
